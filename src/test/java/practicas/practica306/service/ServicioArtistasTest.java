package practicas.practica306.service;

import practicas.practica306.dao.DaoArtistas;
import practicas.practica306.exceptions.RegistroDuplicado;
import practicas.practica306.model.Artista;
import practicas.practica306.service.ServicioArtistas;
import practicas.practica306.utils.UtilidadesArtista;
import org.junit.Before;
import org.junit.Test;

import java.util.Arrays;
import java.util.List;

import static org.junit.Assert.*;
import static org.mockito.Mockito.*;

public class ServicioArtistasTest {

    private DaoArtistas daoArtistasMock;
    private ServicioArtistas servicioArtistas;

    private UtilidadesArtista utilidadesArtista = new UtilidadesArtista();

    @Before
    public void setUp() {

        daoArtistasMock = mock(DaoArtistas.class);
        servicioArtistas = new ServicioArtistas(daoArtistasMock);
    }

    @Test
    public void agregarArtista_exito() throws Exception {
        Artista artista = new Artista("Artista1", 1000.0, utilidadesArtista.convertStringToLocalDate("1980-01-01"));
        doNothing().when(daoArtistasMock).agregarArtista(artista);

        servicioArtistas.agregarArtista(artista);

        verify(daoArtistasMock, times(1)).agregarArtista(artista);
    }

    @Test(expected = RegistroDuplicado.class)
    public void agregarArtista_registroDuplicado() throws Exception {
        Artista artista = new Artista("Artista1", 1000.0, utilidadesArtista.convertStringToLocalDate("1980-01-01"));
        doThrow(new RegistroDuplicado("El artista ya existe")).when(daoArtistasMock).agregarArtista(artista);

        servicioArtistas.agregarArtista(artista);
    }

    @Test
    public void mostrarArtistas_exito() throws Exception {
        List<Artista> artistas = Arrays.asList(
                new Artista("Artista1", 1000.0, utilidadesArtista.convertStringToLocalDate("1980-01-01")),
                new Artista("Artista2", 2000.0, utilidadesArtista.convertStringToLocalDate("1990-01-01"))
        );
        when(daoArtistasMock.mostrarArtistas()).thenReturn(artistas);

        List<Artista> resultado = servicioArtistas.mostrarArtistas();

        assertEquals(2, resultado.size());
        assertEquals("Artista1", resultado.get(0).getNombre());
        verify(daoArtistasMock, times(1)).mostrarArtistas();
    }

    @Test
    public void tieneArtista_artistaExiste() throws Exception {
        String nombreArtista = "Artista1";
        when(daoArtistasMock.tieneArtista(nombreArtista)).thenReturn(true);

        boolean resultado = servicioArtistas.tieneArtista(nombreArtista);

        assertTrue(resultado);
        verify(daoArtistasMock, times(1)).tieneArtista(nombreArtista);
    }

    @Test
    public void eliminarArtista_exito() throws Exception {
        String nombreArtista = "Artista1";
        when(daoArtistasMock.eliminarArtista(nombreArtista)).thenReturn(true);

        boolean resultado = servicioArtistas.eliminarArtista(nombreArtista);

        assertTrue(resultado);
        verify(daoArtistasMock, times(1)).eliminarArtista(nombreArtista);
    }

    @Test
    public void actualizarArtista_exito() throws Exception {
        String nombreActual = "Artista1";
        Artista nuevoArtista = new Artista("Artista2", 1500.0, utilidadesArtista.convertStringToLocalDate("1985-01-01"));
        when(daoArtistasMock.actualizarArtista(nombreActual, nuevoArtista)).thenReturn(true);

        boolean resultado = servicioArtistas.actualizarArtista(nombreActual, nuevoArtista);

        assertTrue(resultado);
        verify(daoArtistasMock, times(1)).actualizarArtista(nombreActual, nuevoArtista);
    }

    @Test
    public void eliminarTodosArtistas_exito() throws Exception {
        doNothing().when(daoArtistasMock).eliminarTodosArtistas();

        servicioArtistas.eliminarTodosArtistas();

        verify(daoArtistasMock, times(1)).eliminarTodosArtistas();
    }
}
